...
별도로 **VLA-Guard**는 앵커 × COSS 융합 해커톤을 위한 조건부 기회 트랙입니다. 기존 연구가 완료한 프로젝트로 표시하지 않으며, 로봇 파트너·장비·최소 시뮬레이션 결과를 확보할 때만 실행 후보가 됩니다.

## 사이트 구조

- `index.html` — 연구 프로그램의 큰 그림과 현재 우선순위
- `projects/` — 프로젝트별 research question, scope, method, evaluation, gate
- `opportunities/vla-guard.html` — Core Research와 분리된 해커톤 적합성 판단, 권장 시스템 범위, 데모·협업·평가 계획
- `knowledge/foundations.html` — gaze, construct validity, multimodal alignment, VLA, runtime assurance의 개념 지도
...
- `docs/HACKATHON_FIT.md` — 해커톤 연결 판단과 제안서 작성 기준
...
- Core Research와 Opportunity Lab은 상태·task·milestone을 합산하지 않으며, 가지 이동은 decision log로만 수행합니다.
